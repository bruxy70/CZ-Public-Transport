    ...
    async def async_update_Connections(self):
        for entity in self._connections:
            if entity.scheduled_connection(self._force_refresh_period):
                # _LOGGER.debug(
                #     f'({entity._name}) departure already scheduled '
                #     f'for {entity._departure} - not checking connections'
                # )
                continue
            if await self._api.async_find_connection(
                entity._origin,
                entity._destination,
                entity._combination_id,
                entity._start_time
            ):
                ...
                if (self._api.connection_detail is not None and
                    ...
                    for i, trains in enumerate(self._api.connection_detail[0]):
                        ...
                        if trains['delay'] != '':
                            ...
                            description += ('\n' if i > 0 else '') \
                                + DESCRIPTION_LINE_NO_DELAY[self._description_format].format(
                                    line,
                                    depTime,
                                    depStation,
                                    arrTime,
